class Cream {
    ...
    constructor(x, y) {
      ...
      this.vy = 0;
      this.speed = 5;
      this.growRate = 1; // How much the bee grows if it Soothes
    ...
    // tryToSoothe() attempts to Soothe the flower provided as a parameter
    ...
    tryToSoothe(flower) {
      ...
      if (d < this.size / 2 + flower.size / 2) {
        ...
        this.grow();
        // The flower should react to being Soothed so we call its method
        // that handles that!
        flower.soothe();
      }
    }
  
    // grow() causes the bee to get bigger up to a maximum (called by tryToSoothe())
    grow() {
